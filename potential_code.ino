...
void setup(void){
  ...
  */
  ...
  {
    Serial.println("This don't work!");
  }
}
...
void readPressure() {
  ...
  currentPressure = (sensorVoltage); 
  Serial.println(currentPressure);

  flightData = SD.open(filename, FILE_WRITE);
  if (flightData)
  {
    Serial.println("This is workin'");
    
    sprintf(buffer, "If you see this, something is going right, pressure: %f", currentPressure);  //launchtime: %f\n\n", currentPressure, launchTime);
    Serial.println(buffer);
    flightData.println(buffer);
    delay(100);
    memset(buffer, 0, sizeof(buffer));
    
  } else{
    Serial.println("This is not openin'");
  }
  if(iterator % 3 == 0)
  {
    flightData.close();
    Serial.println("Hey, I'm closin'");
  }
  iterator = iterator + 1;
}